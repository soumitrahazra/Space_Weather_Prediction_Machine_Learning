...
xs, ys = df.loc[:, 'col1':'col7'], df['col120'] #X <- 1st column name : last column name, y <- label column name
...
def classification_report_with_accuracy_score(y_true, y_pred):
    ...
    return a,Pr,Pr1,Re,Re1,F1,F11,HSS_1, HSS_2, GS, TSS, TP, TN, FN, FP
...
#standardization (almost like normalizing)  
    ...
    classifier_logistic=LogisticRegression(solver= 'lbfgs' , class_weight= 'balanced')
    classifier_logistic.fit(X_train, y_train)
    y_pred = classifier_logistic.predict(X_test)
    a,Pr,Pr1,Re,Re1,F1,F11,HSS_1, HSS_2, GS, TSS, TP, TN, FN, FP=classification_report_with_accuracy_score(y_test, y_pred)
    a_f=a_f+[a]
    Pr_f=Pr_f+[Pr]
    Pr1_f=Pr1_f+[Pr1]
    Re_f=Re_f+[Re]
