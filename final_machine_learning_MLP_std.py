...
model1.add(Dense(units=60,input_dim=119, activation="relu",kernel_regularizer=regularizers.l2(0.01),name="first"))
model1.add(Dense(units=30,activation="relu",name="second"))
...
xs, ys = df.loc[:, 'col1':'col119'], df['col120']
...
def classification_report_with_accuracy_score(y_true, y_pred):
    ...
    return a,Pr,Pr1,Re,Re1,F1,F11,HSS_1, HSS_2, GS, TSS, TP, FP, TN, FN
...
for i in range(12):
    ...
    #model1.fit(X_train, y_train,epochs=500,batch_size=100, verbose=0)
    ...
    a,Pr,Pr1,Re,Re1,F1,F11,HSS_1, HSS_2, GS, TSS, TP, FP, TN, FN=classification_report_with_accuracy_score(y_test, predicted_test)
    a_f=a_f+[a]
    Pr_f=Pr_f+[Pr]
    Pr1_f=Pr1_f+[Pr1]
    Re_f=Re_f+[Re]
